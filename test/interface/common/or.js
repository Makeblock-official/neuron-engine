...
var boolean_datadriven = [
  ...
  {'number1' : -1, 'number2' : 20, 'result' : true, 'portName' : 'result'},   // true false => true
  {'number1' : 8, 'number2' : 0, 'result' : true, 'portName' : 'result'},   // false true => true
...
var number_datadriven = [
  ...
  {'number1' : -1, 'number2' : 20, 'result' : true, 'portName' : 'result'},   // false true => true
  {'number1' : 8, 'number2' : 0, 'result' : true, 'portName' : 'result'},   // true false => true
  {'number1' : 9, 'number2' : 1, 'result' : true, 'portName' : 'result'},   // true true => true
];

// NOTES: data range will effect the final result.
// because when input port is not changed, it will not excute not computing!

var multiple_datadriven = [
  {'number1' : 0, 'number2' : 0, 'result' : true, 'portName' : 'result' },   // negtive true => true
  {'number1' : 8, 'number2' : 7, 'result' : true, 'portName' : 'result'},   // positive false => true
  {'number1' : 3, 'number2' : -9, 'result' : true, 'portName' : 'result'},   // positive true => true
  {'number1' : -6, 'number2' : 1, 'result' : true, 'portName' : 'result'},   // negtive false => true
];
...
describe('OR node', function(){
  ...
  boolean_datadriven.forEach(function(data){
    it('or_003 : boolean inputs computing with or', function(){
      var clock = sinon.useFakeTimers();
      or_node_id = engine.addNode("OR");
      clock.tick(500);
      ...
      clock.tick(10);
      engine.connect(not1_node_id, 'c', or_node_id, 'a');
      clock.tick(10);
      engine.connect(not2_node_id, 'c', or_node_id, 'a');
      ...
      expect(cb_out_node_id).to.be.eql(or_node_id);
  ...
  number_datadriven.forEach(function(data){
    it('or_004 : number inputs computing with or', function(){
      var clock = sinon.useFakeTimers();
      or_node_id = engine.addNode("OR");
      clock.tick(500);
      number1_node_id = engine.addNode("NUMBER");
      ...
      engine.connect(number1_node_id, 'number', or_node_id, 'a');
      clock.tick(10);
      engine.connect(number2_node_id, 'number', or_node_id, 'a');
  ...
  multiple_datadriven.forEach(function(data){
    it('or_005 1: multiple inputs computing with or', function(){
      var clock = sinon.useFakeTimers();
      or_node_id = engine.addNode("OR");
      clock.tick(500);
      number1_node_id = engine.addNode("NUMBER");
      ...
      engine.connect(not2_node_id, 'c', or_node_id, 'a');
      clock.tick(10);
      engine.connect(led_color_picker, 'color', or_node_id, 'a');
  ...
  multiple_datadriven.forEach(function(data){
    it('or_005 2: multiple inputs computing with or', function(){
      var clock = sinon.useFakeTimers();
      or_node_id = engine.addNode("OR");
      clock.tick(500);
      number1_node_id = engine.addNode("NUMBER");
