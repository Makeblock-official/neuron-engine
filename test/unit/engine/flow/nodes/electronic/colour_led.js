...
describe('COLOUR_LED node', function(){
  var enine;
  var driver;
  var id;
  ...
  before(function() {
    ...
    for (var ID in _activeNodeCache){
      if (_activeNodeCache[ID].type === "COLOUR_LED"){
        id = ID;
      }
    }
  });
...
*/
  ...
  it('When input changed, should report to app and send to block', function() {
    for (var i = 0; i < configCases.length; i++){
      _activeNodeCache[id].updateInput('color',configCases[i].color,true);
