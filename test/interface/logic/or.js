...
describe('OR node', function(){
  ...
  it('Input ports can be changed by external input', function(){
    for (var i = 0; i < inputPortPkgs.length; i++) {
      ...
      engine.connect(number_node_id,'number',or_node_id,inputPortPkgs[i].input_port);
      // verify in port xxx value changed
      expect(cb_in_node_id).to.be.eql(or_node_id);
  ...
  it('Output c can be exported to other node', function(){
    ...
    clock.tick(2);
    or2_node_id = engine.addNode("OR");
    console.log("or_node_id: " + or_node_id);
    console.log("number_a_id: " + number_a_id);
    console.log("number_b_id: " + number_b_id);
    console.log("or2_node_id: " + or2_node_id);

    engine.configNode(number_a_id,{'number' : 0});
    // resetParams(cb_out_node_id,cb_out_name,cb_out_value);
    engine.connect(number_a_id,'number',or_node_id,'a');
    // resetParams(cb_in_node_id,cb_in_name,cb_in_value);
    engine.configNode(number_b_id,{'number' : 1});
    engine.connect(number_b_id,'number',or_node_id,'b');

    expect(cb_out_node_id).to.be.eql(or_node_id);
    expect(cb_out_name).to.be.eql('c');
    expect(cb_out_value).to.be.eql(100);

    engine.connect(or_node_id,'c',or2_node_id,'a');

    expect(cb_in_node_id).to.be.eql(or2_node_id);
    expect(cb_in_name).to.be.eql('a');
    expect(cb_in_value).to.be.eql(100);

    engine.connect(or_node_id,'c',or2_node_id,'b');

    expect(cb_in_node_id).to.be.eql(or2_node_id);
    expect(cb_in_name).to.be.eql('b');
    expect(cb_in_value).to.be.eql(100);

    expect(cb_out_node_id).to.be.eql(or2_node_id);
    expect(cb_out_name).to.be.eql('c');
    expect(cb_out_value).to.be.eql(100);
  ...
  it('Remove an or node', function(){
    or_node_id = engine.addNode("OR");
    node = engine.getActiveNodeCache();
    expect(node).to.include.keys(or_node_id);
    // console.log("AAA: " + JSON.stringify(node));
    engine.removeNode(or_node_id);
    node = engine.getActiveNodeCache();
