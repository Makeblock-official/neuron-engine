...
describe('SINGLE_DC_MOTOR node', function(){
  var enine;
  var driver;
  var id;
  ...
  before(function() {
    ...
    for (var ID in _activeNodeCache){
      if (_activeNodeCache[ID].type === "SINGLE_DC_MOTOR"){
        id = ID;
      }
    }
  });
...
*/
  ...
  it('When input changed, should report to app and send to block', function() {
    for (var i = 0; i < motorCases.length; i++){
      _activeNodeCache[id].updateInput('motor',motorCases[i].motor,true);
      expect(inId).to.be.eql(id);
      expect(inPort).to.be.eql(motorCases[i].wantInPort); 
      expect(inValue).to.be.eql(motorCases[i].wantInValue);      
    }
  });
});
